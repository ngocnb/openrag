...
@/home/baongoc/workspaces/openrag/.claude/get-shit-done/references/ui-brand.md
</required_reading>
...
```bash
PHASE_INFO=$(node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_DESC=$(node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" | jq -r '.section')
```
...
**If found:**
1. Read template: `/home/baongoc/workspaces/openrag/.claude/get-shit-done/templates/VALIDATION.md`
...
```bash
UI_PHASE_CFG=$(node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
2. Disable Nyquist with the exact command:
   `node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow.nyquist_validation false`
...
- **`## ISSUES FOUND`:** Display issues, check iteration count, proceed to step 14.

## 12. Context Decisions Coverage Gate (MANDATORY)

**This step is MANDATORY — it catches the "spec says X, plan forgets X" problem.**

Read CONTEXT.md and extract all items from the `<decisions>` section. These represent locked decisions that MUST be addressed by the plans.

**Step 1: Extract decisions from CONTEXT.md**

```bash
# Extract decision items (lines starting with "- " under <decisions>)
DECISIONS=$(grep -A 100 "<decisions>" "${CONTEXT_PATH}" 2>/dev/null | grep -B 100 "</decisions>" | grep -E "^- " | sed 's/^- //')
```

**Step 2: For each decision, check if it appears in any plan**

For each decision item:
1. Extract key terms (e.g., "Add Alibaba to onboarding flow" → keywords: "alibaba", "onboarding")
2. Search all PLAN.md files for these keywords in `<objective>`, `<action>`, or task descriptions
3. If no match found → flag as uncovered

**Step 3: Report coverage**

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GSD ► CONTEXT DECISIONS COVERAGE CHECK
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Decision Coverage: {covered}/{total}
```

**If all decisions covered:**
```
✓ All CONTEXT.md decisions are addressed by plans
```
→ Proceed to step 13.

**If gaps found:**
```
## ⚠ Context Decisions Coverage Gap

The following CONTEXT.md decisions are NOT addressed by any plan task:

| Decision | Status |
|----------|--------|
| {decision_text} | ❌ Not covered |
| {decision_text} | ❌ Not covered |

**This means the plan is incomplete.** The spec defined these requirements but the planner dropped them.

Options:
1. **Revise plans** (recommended) — Return to planner to add missing tasks
2. **Remove from CONTEXT.md** — Decision was wrong, remove it
3. **Defer to future phase** — Move decision to a later phase

**You cannot proceed without resolving this gap.**
```

Use AskUserQuestion to present options:
- Option 1: Spawn gsd-planner with revision prompt listing missing decisions
- Option 2: Allow user to edit CONTEXT.md, then re-run coverage check
- Option 3: Move to deferred, update CONTEXT.md, continue

**IMPORTANT:** If user selects Option 1 (revise), after planner returns, loop back to step 10 (plan-checker) to re-verify. Do NOT skip this loop — revised plans must pass the checker again.
...
**Missing context decisions:** {uncovered_decisions_from_step_12}
</revision_context>

<instructions>
Make targeted updates to address checker issues AND missing context decisions.
...
## 14. Requirements Coverage Gate

After plans pass the checker and context decisions gate, verify that all phase requirements are covered by at least one plan.
...
2. **Sync chain flag with intent** — if user invoked manually (no `--auto`), clear the ephemeral chain flag from any previous interrupted `--auto` chain. This does NOT touch `workflow.auto_advance` (the user's persistent settings preference):
   ...
   if [[ ! "$ARGUMENTS" =~ --auto ]]; then
     node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
   fi
   ```
3. Read both the chain flag and user preference:
   ```bash
   AUTO_CHAIN=$(node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
   AUTO_CFG=$(node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
   ```
...
- [ ] Context Decisions Coverage Gate passed (step 12) — ALL CONTEXT.md decisions covered by plans
