...
```bash
INIT=$(node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" init execute-phase "${PHASE_ARG}")
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `/home/baongoc/workspaces/openrag/.claude/get-shit-done/workflows/execute-plan.md` **inline**
...
```bash
node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
2. **Spawn executor agents:**
   ...
   Task(
     ...
     prompt="
       ...
       @/home/baongoc/workspaces/openrag/.claude/get-shit-done/references/checkpoints.md
       @/home/baongoc/workspaces/openrag/.claude/get-shit-done/references/tdd.md
       </execution_context>
...
5b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
PARENT_INFO=$(node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" find-phase "${PARENT_PHASE}" --raw)
...
```bash
node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
```bash
COMPLETION=$(node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" phase complete "${PHASE_NUMBER}")
```
...
```bash
node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
```bash
node "/home/baongoc/workspaces/openrag/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
Read and follow `/home/baongoc/workspaces/openrag/.claude/get-shit-done/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
